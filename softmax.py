...
class SoftmaxRegression:
    ...
    def cost(self, X, Y, W=None):
       ...
       # W_X = W_X - np.max(W_X)
        ...
        new_W_X = np.exp(W_X)
        probabilities = np.divide(new_W_X, np.sum(new_W_X, axis = 0))

        cost = np.multiply(-1,np.sum(np.multiply(np.log(probabilities),indicator))/N)
    ...
    def predict(self, X):
        ...
        new_W_X = np.exp(W_X)
        probabilities = new_W_X / np.sum(new_W_X, axis = 0)
    ...
    def main(self,X,Y,Xtest,Ytest):
        import runClassifier
        runClassifier.learningCurve(self.classifier, self.numClasses, self.exSize, X, Y, Xtest, Ytest)
