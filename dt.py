    ...
    def trainDT(self, X, Y, maxDepth, used):
        ...
        if maxDepth <= 0 or len(util.uniq(Y)) <= 1:
            ...
            for d in range(D):
                ...
                count = 0
                for item in leftY:
                    if item != util.mode(leftY):
                        count += 1
                for item in rightY:
                    if item != util.mode(rightY):
                        count += 1
                error =  count 
                """
                arr_left = [util.mode(leftY)]*len(leftY)
                arr_right = [util.mode(rightY)]*len(rightY)
                error = sum((leftY != arr_left)) + sum((rightY != arr_right))
            ...
            if bestFeature < 0:
                ...
                self.left.trainDT(X[X[:, self.feature] < 0.5], Y[X[:, self.feature] < 0.5], (maxDepth - 1), used+[self.feature])
                self.right.trainDT(X[X[:, self.feature] >= 0.5], Y[X[:, self.feature] >= 0.5], (maxDepth - 1), used+[self.feature])
